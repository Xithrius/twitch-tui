...
impl Data {
    pub fn new(author: &str, system: bool, message: String) -> Self {
        Self {
            time_sent: Local::now(),
...
impl<'conf> DataBuilder<'conf> {
    ...
    pub fn user(user: String, message: String) -> Data {
        Data::new(user.as_str(), false, message)
    }

    pub fn system(self, message: String) -> Data {
        Data::new("System", true, message)
    }

    pub fn twitch(self, message: String) -> Data {
        Data::new("Twitch", true, message)
    }
}
...
mod tests {
    ...
    fn test_username_hash() {
        assert_eq!(
            Data::new("human", false, "beep boop".to_string()).hash_username(&Palette::Pastel),
            Rgb(159, 223, 221)
        );
    }
}
